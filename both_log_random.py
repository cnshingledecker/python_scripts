...
for j in range(0,Nruns):
    ...
    #Calculate new initial OPR values
    r1 = random.random()
    ...
    resf = open(results,'a') #Open the results file
    resf.write(temp_res) #Write the OPR data
    
    # Calculate new Zeta value
    r2 = random.random()
    temp_zeta = 10**(r1*math.log10(zeta_min) + (1-r1)*math.log10(zeta_max)) 
    temp_res2 =  str(temp_zeta)  + '\n'
    resf.write(temp_res2) #Write the zeta data
    resf.close() #Close the results file
